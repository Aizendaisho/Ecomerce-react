...
export default function ShoppingCart({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <div
      onClick={() => closeCart()}
      className={` z-20 absolute top-0 right-0 w-full h-full bg-gray-900 text-white opacity-90 ${
        isOpen ? "" : "hidden"
      }`}
    >
      <div
        onClick={(e) => hijo(e)}
        className={`z-30 absolute top-0 right-0 w-2/4 h-full bg-white text-black `}
      >
        <button
          onClick={() => closeCart()}
          className=" z-40 text-3xl px-4 py-2 bg-gray-600 top-2 right-2 absolute text-white"
        >
          x
        </button>
